...
const unsigned long TEMPO_ESPERA = 10000; //10S minutos
  ...
  // {
    ...
    if(lorawan.isConnected()){  //Verifica se o modulo esta conectado
      ...
      if(intervalo < millis()){
        ...
        // Cria objetos JSON dinâmicos para cada ALIAS
      //   DynamicJsonDocument jsonLDR(JSON_OBJECT_SIZE(1));
      //   DynamicJsonDocument jsonTemperatura(JSON_OBJECT_SIZE(1));
      //   DynamicJsonDocument jsonUmidade(JSON_OBJECT_SIZE(1));

      //   // Adiciona os valores aos objetos JSON individuais
      //   jsonLDR[ALIAS] = leitura_LDR;
      //   jsonTemperatura[ALIAS1] = temperatura;
      //   jsonUmidade[ALIAS2] = umidade;

      //   // Serializa cada objeto JSON individualmente
      //   char mensagemLDR[measureJson(jsonLDR) + 1];
      //   serializeJson(jsonLDR, mensagemLDR, sizeof(mensagemLDR));
      //   //Imprime a mensagem e envia a mensagem como texto pelo modulo
      //   Serial.print(F("Mensagem: "));
      //   Serial.println(mensagemLDR);
      //   resposta = lorawan.sendT(1, mensagemLDR); //Envio como texto  

      //   char mensagemTemperatura[measureJson(jsonTemperatura) + 1];
      //   serializeJson(jsonTemperatura, mensagemTemperatura, sizeof(mensagemTemperatura));
      //   //Imprime a mensagem e envia a mensagem como texto pelo modulo
      //   Serial.print(F("Mensagem: "));
      //   Serial.println(mensagemTemperatura);
      //   resposta = lorawan.sendT(1, mensagemTemperatura); //Envio como texto  

      //   char mensagemUmidade[measureJson(jsonUmidade) + 1];
      //   serializeJson(jsonUmidade, mensagemUmidade, sizeof(mensagemUmidade));
      //  //Imprime a mensagem e envia a mensagem como texto pelo modulo
      //   Serial.print(F("Mensagem: "));
      //   Serial.println(mensagemUmidade);
      //   resposta = lorawan.sendT(1, mensagemUmidade); //Envio como texto  
